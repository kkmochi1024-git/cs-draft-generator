...
| `label` | `str` | PII種別ラベル（`EMAIL`/`PHONE`/`ZIPCODE`、またはGiNZAのエンティティラベル `Person`/`Province`/`City`/`Country`） |
...
- 処理の要点:
  - `PRIORITY = ["EMAIL", "ZIPCODE", "PHONE"]` の順でパターンを走査し、先に検出した範囲を `occupied` に記録する
...
- 検出対象エンティティラベル（`TARGET_LABELS`）: `Person`, `Province`, `City`, `Country`。GiNZAは地名を `Location` ではなく `Province`/`City`/`Country` 等で返すため、これらを個別に列挙している
...
- 処理の要点:
  ...
  6. 正規表現エンティティとNERエンティティ（元テキスト位置に変換済み）を合算し、開始位置の**降順**にソートしてから元テキストへ置換を適用する。末尾から置換することで、前方の置換によって後方の位置情報がずれる問題を回避する
  7. `mapping` はトークン→元の値の辞書として構築し、最終的に `entities` は開始位置昇順に再ソートして返す
...
flowchart TD
    ...
    G --> H["regex_entities + 変換済みner_entities を統合し<br/>開始位置の降順でソート"]
